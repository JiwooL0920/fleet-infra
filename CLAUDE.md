...
- **Grafana admin password**: LocalStack regenerates a random password on any pod recreation, which propagates through `grafana-admin-credentials` ExternalSecret. Grafana's persistent DB retains the ORIGINAL password from first init, causing 401s. `create-grafana-sa-token` Job now includes a `reset-admin-password` initContainer that runs `grafana cli admin reset-admin-password` on every reconcile, making the K8s Secret the single source of truth. Self-heals on every colima restart without manual intervention.

**Colima Restart Resilience**:
- **PostgreSQL cluster**: `base/services/postgresql-cluster.yaml` declares `external-secrets-config` as a dependency so the `cnpg-backup-s3` ExternalSecret's ClusterSecretStore is guaranteed Ready before the CNPG Cluster CR is applied. Without this, cold-boot ordering can time out the 15-min health check.
- **kagent code-agent**: Pins `runtime: go` explicitly in the Agent CR — Python default SIGILLs on arm64 (Apple Silicon Colima). Python code execution moved to the code-tools-mcp-server pod. All other agents already pin `runtime: go` for the same reason.
- **kubescape vulnerabilityScan**: Disabled in `apps/base/kubescape/helmrelease.yaml`. Upstream needs 5Gi+ memory for `kubevuln`; dev cluster allocates 1Gi, causing OOM churn. No workload in this repo consumes `VulnerabilityManifest` CRs, so disabling is safe.
