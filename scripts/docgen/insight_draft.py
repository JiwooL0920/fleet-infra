...
Output is written to service-insights/<slug>.suggested.yaml.
Primary insight files are only updated via explicit acceptance.
...
ALLOW_DANGEROUS_ENV = "INSIGHT_DRAFT_ALLOW_DANGEROUS"
...
def run_opencode(prompt_file: Path, context_files: list[Path]) -> str:
    ...
    if os.environ.get(ALLOW_DANGEROUS_ENV) == "1":
        print(
            f"  WARNING: {ALLOW_DANGEROUS_ENV}=1 set; enabling --dangerously-skip-permissions (local dev only)",
            file=sys.stderr,
        )
...
def _validate_operations_safety(drafted: dict) -> None:
    """Reject dangerous commands in operations.steps generated by the LLM."""
    blocked = [
        r"rm\s+-rf",
        r"dd\s+if=",
        r"mkfs",
    ...
    ops = drafted.get("operations")
    if not isinstance(ops, list):
        return

    for idx, scenario in enumerate(ops):
        ...
        for sidx, step in enumerate(steps):
            ...
            for pattern in blocked:
                if re.search(pattern, step, flags=re.IGNORECASE):
                    raise ValueError(
                        f"Rejected unsafe operations step at operations[{idx}].steps[{sidx}] matching pattern '{pattern}': {step}"
                    )
...
def draft_service(slug: str, force: bool = False, fields: list[str] | None = None) -> None:
    ...
    suggested_path = INSIGHTS_DIR / f"{slug}.suggested.yaml"
    ...
    try:
        ...
        try:
            _validate_operations_safety(drafted)
        except ValueError as e:
            print(f"  ERROR: {e}", file=sys.stderr)
            return

        merged = merge_into_stub(stub_path, drafted, force=force, only_fields=fields)
        suggested_path.write_text(
            yaml.dump(merged, allow_unicode=True, default_flow_style=False, width=100, sort_keys=False)
        )
        print(f"  Wrote suggestion: {suggested_path.relative_to(REPO_ROOT)}", file=sys.stderr)
...
def main() -> None:
    ...
    print(
        "\nDone. Suggested files were written as service-insights/<svc>.suggested.yaml. "
        "Review, then run 'make insight-accept SVC=<svc>' to promote.",
        file=sys.stderr,
    )
